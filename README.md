...
```text
PicBoard-v0.1.2-windows-x64.zip
```
...
|---|---|
| 展开画板 | 短点悬浮球 |
| 拖动还原 | 最大化时拖动标题栏空白区域，恢复小窗并继续移动 |
| 最大化 / 还原 | 点击标题栏方框按钮铺满当前显示器工作区，再次点击恢复原小窗大小和位置 |
| 移动悬浮球 | 按住约 250ms，或按下后移动 |
| 导入图片 | 拖到悬浮球或展开的画板 |
| 粘贴图片 | 展开画板后按 `Ctrl+V` |
| 缩放画布 | 滚轮向上放大、向下缩小，以鼠标位置为中心，范围 50%–150% |
...
图片操作位于卡片底部的内联操作条，不再向右扩展侧栏。首页画布右键展开，分类库和预览点击小号“⋯”展开；分类按钮将操作条原地切换为分类标签，重命名在同一区域输入并保存，均不另开弹窗。只有分类库删除保留确认。选择/框选不自动展开，可从选择工具条进入批量操作。画布滚轮缩放不影响预览与菜单内滚动，并在滚动结束后保存视角。

在输入框中使用快捷键时，PicBoard 会保留正常的文本编辑行为。

v0.1.2 包含内联图片操作、画布滚轮缩放、窗口最大化/还原与标题栏拖动还原，以及系统回收站路径兼容修复。本地便携包位于 `release/`；GitHub 下载以 Releases 页面实际发布的版本为准。自动化测试不替代 Windows E2E；本轮最终真机交互验收由使用者执行，快速拖动/点击的稳定性尚未完成真机验收。
...
便携包目录不是默认图库目录。升级到 PicBoard v0.1.2 不会迁移或清空旧数据。重要图片仍建议自行备份。
...
```powershell
Get-FileHash .\PicBoard-v0.1.2-windows-x64.zip -Algorithm SHA256
```

输出应与 Release 页面和 `PicBoard-v0.1.2-windows-x64.manifest.json` 一致。
